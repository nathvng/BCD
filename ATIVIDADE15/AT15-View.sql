...
FROM tb_cliente INNER JOIN tb_produto
ON tb_produto.estoque < 50
WHERE tb_cliente.estado IN ('SP', 'RJ', 'MG', 'ES')
...
SELECT nome_cliente, telefone_cliente, CASE mes_nascimento
WHEN 1 THEN 'JANEIRO'
WHEN 3 THEN 'MARÇO'
WHEN 5 THEN 'MAIO'
WHEN 7 THEN 'JULHO'
...
SELECT tb_cliente.nome_cliente, SUM(tb_produto.lucro) AS lucro_total
FROM vw_produtos_lucro INNER JOIN vw_clientes_mais_lucrativos
ON tb_produto.categoria = 'Alimentos'
GROUP BY tb_cliente.nome_cliente HAVING lucro_total > 100
